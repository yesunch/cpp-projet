...
class NetworkAudioStream : public sf::SoundStream
{
    ...
    void start(unsigned short port)
    {
        if (!m_hasFinished)
        {
            ...
            if (m_listener.listen(port) != sf::Socket::Done)
                return;
            std::cout << "Server is listening to port " << port << ", waiting for connections... "
                      << std::endl;
  ...
  private:
    ...
    {
        // We have reached the end of the buffer and all audio data have been played: we can stop
        // playback
