...
////////////////////////////////////////////////////////////
Socket::Status
TcpSocket::connect(const IpAddress& remoteAddress, unsigned short remotePort, Time timeout)
{
...
////////////////////////////////////////////////////////////
Socket::Status
TcpSocket::send(const void* data, std::size_t size)
{
    if (!isBlocking())
...
////////////////////////////////////////////////////////////
Socket::Status
TcpSocket::send(const void* data, std::size_t size, std::size_t& sent)
{
...
TcpSocket::send(Packet& packet)
{
    ...
    std::size_t sent;
    Status status =
        send(&blockToSend[0] + packet.m_sendPos, blockToSend.size() - packet.m_sendPos, sent);
...
TcpSocket::receive(Packet& packet)
{
    ...
    if (m_pendingPacket.SizeReceived < sizeof(m_pendingPacket.Size))
    {
        ...
        {
            char* data =
                reinterpret_cast<char*>(&m_pendingPacket.Size) + m_pendingPacket.SizeReceived;
            Status status = receive(
                data, sizeof(m_pendingPacket.Size) - m_pendingPacket.SizeReceived, received);
    ...
    while (m_pendingPacket.Data.size() < packetSize)
    {
        ...
        std::size_t sizeToGet = std::min(
            static_cast<std::size_t>(packetSize - m_pendingPacket.Data.size()), sizeof(buffer));
