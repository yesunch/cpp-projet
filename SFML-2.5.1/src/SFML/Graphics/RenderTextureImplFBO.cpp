...
destroyStaleFBOs()
{
    ...
    for (std::set<std::pair<sf::Uint64, unsigned int>>::iterator iter = staleFrameBuffers.begin();
...
void
contextDestroyCallback(void* arg)
{
    sf::Lock lock(mutex);
    ...
    for (std::set<std::map<sf::Uint64, unsigned int>*>::iterator frameBuffersIter =
             frameBuffers.begin();
         frameBuffersIter != frameBuffers.end();
         ++frameBuffersIter)
    {
        for (std::map<sf::Uint64, unsigned int>::iterator iter = (*frameBuffersIter)->begin();
             iter != (*frameBuffersIter)->end();
             ++iter)
        {
            if (iter->first == contextId)
            {
...
RenderTextureImplFBO::~RenderTextureImplFBO()
{
    ...
    for (std::map<Uint64, unsigned int>::iterator iter = m_frameBuffers.begin();
         iter != m_frameBuffers.end();
         ++iter)
        staleFrameBuffers.insert(std::make_pair(iter->first, iter->second));

    for (std::map<Uint64, unsigned int>::iterator iter = m_multisampleFrameBuffers.begin();
         iter != m_multisampleFrameBuffers.end();
         ++iter)
        staleFrameBuffers.insert(std::make_pair(iter->first, iter->second));
...
////////////////////////////////////////////////////////////
bool
RenderTextureImplFBO::create(unsigned int width,
                             unsigned int height,
...

    {
        ...
        if (!settings.antialiasingLevel)
        {
            ...
            if (settings.stencilBits)
            {
                ...
                if (!m_depthStencilBuffer)
                {
                    err() << "Impossible to create render texture (failed to create the attached "
        ...
        else
        {
            ...
            glCheck(GLEXT_glRenderbufferStorageMultisample(
                GLEXT_GL_RENDERBUFFER, settings.antialiasingLevel, GL_RGBA, width, height));
            ...
            if (settings.stencilBits)
            {
                ...
                {
                    err() << "Impossible to create render texture (failed to create the attached "
                             "multisample depth/stencil buffer)"
                          << std::endl;
                    return false;
                }
                glCheck(GLEXT_glBindRenderbuffer(GLEXT_GL_RENDERBUFFER, m_depthStencilBuffer));
            ...
            err() << "Impossible to create render texture (failed to create the multisample render "
                     "buffers)"
                  << std::endl;
            return false;
...
RenderTextureImplFBO::createFrameBuffer()
{
    ...
    glCheck(GLEXT_glFramebufferTexture2D(
        GLEXT_GL_FRAMEBUFFER, GLEXT_GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, m_textureId, 0));
    ...
    if (status != GLEXT_GL_FRAMEBUFFER_COMPLETE)
    {
        glCheck(GLEXT_glBindFramebuffer(GLEXT_GL_FRAMEBUFFER, 0));
        glCheck(GLEXT_glDeleteFramebuffers(1, &frameBuffer));
        err() << "Impossible to create render texture (failed to link the target texture to the "
                 "frame buffer)"
              << std::endl;
        return false;
    }

    {
        ...
        m_frameBuffers.insert(
            std::make_pair(Context::getActiveContextId(), static_cast<unsigned int>(frameBuffer)));
    }
    ...
    if (m_multisample)
    {
        ...
        {
            err() << "Impossible to create render texture (failed to create the multisample frame "
                     "buffer object)"
                  << std::endl;
            return false;
        }
        ...
        if (status != GLEXT_GL_FRAMEBUFFER_COMPLETE)
        {
            ...
            err() << "Impossible to create render texture (failed to link the render buffers to "
                     "the multisample frame buffer)"
                  << std::endl;
            return false;
        }

        {
            ...
            // Insert the FBO into our map
            m_multisampleFrameBuffers.insert(std::make_pair(
                Context::getActiveContextId(), static_cast<unsigned int>(multisampleFrameBuffer)));
        }
    }
...
////////////////////////////////////////////////////////////
bool
RenderTextureImplFBO::activate(bool active)
{
    ...
    if (!contextId)
    {
        ...
        if (!contextId)
        {
            err() << "Impossible to activate render texture (failed to create backup context)"
                  << std::endl;
...
////////////////////////////////////////////////////////////
void
RenderTextureImplFBO::updateTexture(unsigned int)
{
    ...
    if (m_multisample && m_width && m_height && activate(true))
    {
        ...
        {
            // Set up the blit target (draw framebuffer) and blit (from the read framebuffer, our
            // multisample FBO)
