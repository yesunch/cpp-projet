...
SoundStream::play()
{
    ...
    if (m_format == 0)
    {
        err() << "Failed to play audio stream: sound parameters have not been initialized (call "
                 "initialize() first)"
              << std::endl;
        return;
    }
...
SoundStream::setPlayingOffset(Time timeOffset)
{
    ...
    // Restart streaming
    m_samplesProcessed =
        static_cast<Uint64>(timeOffset.asSeconds() * m_sampleRate * m_channelCount);
...
SoundStream::getPlayingOffset() const
{
    if (m_sampleRate && m_channelCount)
    {
        ...
        return seconds(secs +
                       static_cast<float>(m_samplesProcessed) / m_sampleRate / m_channelCount);
    }
    else
    {
        return Time::Zero;
    ...
    for (;;)
    {
        ...
        while (nbProcessed--)
        {
            ...
            else
            {
                ...
                if (bits == 0)
                {
                    err() << "Bits in sound stream are 0: make sure that the audio format is not "
                             "corrupt "
