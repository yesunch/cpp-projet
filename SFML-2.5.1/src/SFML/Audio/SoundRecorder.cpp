...
SoundRecorder::~SoundRecorder()
{
    ...
    assert(!m_isCapturing && "You must call stop() in the destructor of your derived class, so "
                             "that the recording thread finishes before your object is destroyed.");
}
...
SoundRecorder::start(unsigned int sampleRate)
{
    ...
    if (captureDevice)
    {
        err() << "Trying to start audio capture, but another capture is already running"
              << std::endl;
        return false;
    }
    ...
    if (!captureDevice)
    {
        err() << "Failed to open the audio capture device with the name: " << m_deviceName
              << std::endl;
        return false;
    }
...
////////////////////////////////////////////////////////////
bool
SoundRecorder::setDevice(const std::string& name)
{
    ...
    if (m_isCapturing)
    {
        ...
        captureDevice =
            alcCaptureOpenDevice(m_deviceName.c_str(), m_sampleRate, format, m_sampleRate);
        if (!captureDevice)
        {
            // Notify derived class
            onStop();
...
////////////////////////////////////////////////////////////
void
SoundRecorder::setChannelCount(unsigned int channelCount)
{
    if (m_isCapturing)
    {
...
////////////////////////////////////////////////////////////
unsigned int
SoundRecorder::getChannelCount() const
{
...
SoundRecorder::isAvailable()
{
    return (priv::AudioDevice::isExtensionSupported("ALC_EXT_CAPTURE") != AL_FALSE) ||
           (priv::AudioDevice::isExtensionSupported("ALC_EXT_capture") !=
            AL_FALSE); // "bug" in Mac OS X 10.5 and 10.6
}

////////////////////////////////////////////////////////////
void
SoundRecorder::setProcessingInterval(Time interval)
{
...
////////////////////////////////////////////////////////////
void
SoundRecorder::processCapturedSamples()
{
