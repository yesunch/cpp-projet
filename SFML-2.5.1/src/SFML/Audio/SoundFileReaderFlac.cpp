...
streamRead(const FLAC__StreamDecoder*, FLAC__byte buffer[], std::size_t* bytes, void* clientData)
{
    sf::priv::SoundFileReaderFlac::ClientData* data =
        static_cast<sf::priv::SoundFileReaderFlac::ClientData*>(clientData);
...
streamSeek(const FLAC__StreamDecoder*, FLAC__uint64 absoluteByteOffset, void* clientData)
{
    sf::priv::SoundFileReaderFlac::ClientData* data =
        static_cast<sf::priv::SoundFileReaderFlac::ClientData*>(clientData);
...
streamTell(const FLAC__StreamDecoder*, FLAC__uint64* absoluteByteOffset, void* clientData)
{
    sf::priv::SoundFileReaderFlac::ClientData* data =
        static_cast<sf::priv::SoundFileReaderFlac::ClientData*>(clientData);
...
streamLength(const FLAC__StreamDecoder*, FLAC__uint64* streamLength, void* clientData)
{
    sf::priv::SoundFileReaderFlac::ClientData* data =
        static_cast<sf::priv::SoundFileReaderFlac::ClientData*>(clientData);
...
FLAC__bool
streamEof(const FLAC__StreamDecoder*, void* clientData)
{
    sf::priv::SoundFileReaderFlac::ClientData* data =
        static_cast<sf::priv::SoundFileReaderFlac::ClientData*>(clientData);
...
streamWrite(const FLAC__StreamDecoder*,
            ...
            void* clientData)
{
    ...
    for (unsigned i = 0; i < frame->header.blocksize; ++i)
    {
        for (unsigned int j = 0; j < frame->header.channels; ++j)
        {
            ...
            {
                // We are either seeking (null buffer) or have decoded all the requested samples
                // during a normal read (0 remaining), so we put the sample in a temporary buffer
                // until next call
...
void
streamMetadata(const FLAC__StreamDecoder*, const FLAC__StreamMetadata* meta, void* clientData)
{
    sf::priv::SoundFileReaderFlac::ClientData* data =
        static_cast<sf::priv::SoundFileReaderFlac::ClientData*>(clientData);
...
void
streamError(const FLAC__StreamDecoder*, FLAC__StreamDecoderErrorStatus, void* clientData)
{
    sf::priv::SoundFileReaderFlac::ClientData* data =
        static_cast<sf::priv::SoundFileReaderFlac::ClientData*>(clientData);
    data->error = true;
...
////////////////////////////////////////////////////////////
bool
SoundFileReaderFlac::check(InputStream& stream)
{
    // Create a decoder
...
////////////////////////////////////////////////////////////
bool
SoundFileReaderFlac::open(InputStream& stream, Info& info)
{
    // Create the decoder
...
////////////////////////////////////////////////////////////
void
SoundFileReaderFlac::seek(Uint64 sampleOffset)
{
    assert(m_decoder);
    ...
    {
        // The "write" callback will populate the leftovers buffer with the first batch of samples
        // from the seek destination, and since we want that data in this typical case, we don't
        // re-clear it afterward
        FLAC__stream_decoder_seek_absolute(m_decoder,
                                           sampleOffset / m_clientData.info.channelCount);
    }
    else
    {
        // FLAC decoder can't skip straight to EOF, so we short-seek by one sample and skip the rest
        FLAC__stream_decoder_seek_absolute(
            m_decoder, (m_clientData.info.sampleCount / m_clientData.info.channelCount) - 1);
        FLAC__stream_decoder_skip_single_frame(m_decoder);

        // This was re-populated during the seek, but we're skipping everything in this, so we need
        // it emptied
        m_clientData.leftovers.clear();
    }
}
...
////////////////////////////////////////////////////////////
Uint64
SoundFileReaderFlac::read(Int16* samples, Uint64 maxCount)
{
    assert(m_decoder);
    ...
    if (left > 0)
    {
        if (left > maxCount)
        {
            ...
            std::copy(m_clientData.leftovers.begin(),
                      m_clientData.leftovers.begin() + static_cast<std::size_t>(maxCount),
                      samples);
    ...
    // Decode frames one by one until we reach the requested sample count, the end of file or an
    // error
