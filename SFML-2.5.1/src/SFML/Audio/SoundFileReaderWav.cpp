...
bool
decode(sf::InputStream& stream, sf::Uint8& value)
{
    return stream.read(&value, sizeof(value)) == sizeof(value);
}

bool
decode(sf::InputStream& stream, sf::Int16& value)
{
...
bool
decode(sf::InputStream& stream, sf::Uint16& value)
{
    unsigned char bytes[sizeof(value)];
    if (stream.read(bytes, sizeof(bytes)) != sizeof(bytes))
        return false;
...
bool
decode24bit(sf::InputStream& stream, sf::Uint32& value)
{
...
bool
decode(sf::InputStream& stream, sf::Uint32& value)
{
    unsigned char bytes[sizeof(value)];
    if (stream.read(bytes, sizeof(bytes)) != sizeof(bytes))
        return false;
...
////////////////////////////////////////////////////////////
bool
SoundFileReaderWav::check(InputStream& stream)
{
...
////////////////////////////////////////////////////////////
bool
SoundFileReaderWav::open(InputStream& stream, Info& info)
{
    m_stream = &stream;
...
////////////////////////////////////////////////////////////
void
SoundFileReaderWav::seek(Uint64 sampleOffset)
{
    assert(m_stream);
...
////////////////////////////////////////////////////////////
Uint64
SoundFileReaderWav::read(Int16* samples, Uint64 maxCount)
{
    assert(m_stream);
...
////////////////////////////////////////////////////////////
bool
SoundFileReaderWav::parseHeader(Info& info)
{
    assert(m_stream);
    ...
    while (!dataChunkFound)
    {
            ...
            (subChunkId[3] == ' '))
        {
            ...
            if (format == waveFormatExtensible)
            {
                ...
                {
                    err() << "Unsupported format: extensible format with non-PCM subformat"
                          << std::endl;
                    return false;
                }
        ...
        else if ((subChunkId[0] == 'd') && (subChunkId[1] == 'a') && (subChunkId[2] == 't') &&
                 (subChunkId[3] == 'a'))
        {
            // "data" chunk
