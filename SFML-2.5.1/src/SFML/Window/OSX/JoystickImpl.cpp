...
{
bool
JoystickButtonSortPredicate(IOHIDElementRef b1, IOHIDElementRef b2)
{
...
std::string
stringFromCFString(CFStringRef cfString)
{
...
std::string
getDeviceString(IOHIDDeviceRef ref, CFStringRef prop, unsigned int index)
{
    ...
    sf::err() << "Unable to read string value for property '" << stringFromCFString(prop)
...
unsigned int
getDeviceUint(IOHIDDeviceRef ref, CFStringRef prop, unsigned int index)
{
    CFTypeRef typeRef = IOHIDDeviceGetProperty(ref, prop);
    ...
    sf::err() << "Unable to read uint value for property '" << stringFromCFString(prop)
              << "' for joystick at index " << index << std::endl;
    return 0;
}
}
            ...
            case kHIDPage_GenericDesktop:
                switch (IOHIDElementGetUsage(element))
                {
                    ...
                    case kHIDUsage_GD_Hatswitch:
                        ...
                        //
                        // We assume this model here as well. Hence, with 4 switches and
                        // intermediate positions we have 8 values (0-7) plus the "null" state (8).
                        {
                            ...
                            if (min != 0 || max != 7)
                            {
                                sf::err() << std::hex << "Joystick (vendor/product id: 0x"
                                          << m_identification.vendorId << "/0x"
                                          << m_identification.productId << std::dec
                    ...
                    case kHIDUsage_GD_GamePad:
                        // We assume a game pad is an application collection, meaning it doesn't
                        // hold any values per say. They kind of "emit" the joystick's usages. See
                        // §3.4.3 Usage Types (Collection) of HUT v1.12
                        if (IOHIDElementGetCollectionType(element) !=
                            kIOHIDElementCollectionTypeApplication)
                        {
                            sf::err()
                                << std::hex << "Gamepage (vendor/product id: 0x"
                                << m_identification.vendorId << "/0x" << m_identification.productId
                                << ") is not an CA but a 0x"
                    ...
                    default:
#ifdef SFML_DEBUG
                        sf::err() << "Unexpected usage for element of Page Generic Desktop: 0x"
                                  << std::hex << IOHIDElementGetUsage(element) << std::dec
                                  << std::endl;
#endif
                        break;
                }
...
JoystickImpl::update()
{
    ...
    for (AxisMap::iterator it = m_axis.begin(); it != m_axis.end(); ++it)
    {
        ...
        float scaledValue = (((physicalValue - physicalMin) * (scaledMax - scaledMin)) /
