...
////////////////////////////////////////////////////////////
bool
CursorImpl::loadFromPixels(const Uint8* pixels, Vector2u size, Vector2u hotspot)
{
    release();
    ...
    for (std::size_t j = 0; j < size.y; ++j)
    {
        for (std::size_t i = 0; i < size.x; ++i)
        {
            ...
            int intensity = pixels[pixelIndex * 4 + 0] + pixels[pixelIndex * 4 + 1] +
    ...
    Pixmap maskPixmap = XCreateBitmapFromData(
        m_display, XDefaultRootWindow(m_display), (char*)&mask[0], size.x, size.y);
    Pixmap dataPixmap = XCreateBitmapFromData(
        m_display, XDefaultRootWindow(m_display), (char*)&data[0], size.x, size.y);
    ...
    m_cursor =
        XCreatePixmapCursor(m_display, dataPixmap, maskPixmap, &fg, &bg, hotspot.x, hotspot.y);
...
////////////////////////////////////////////////////////////
bool
CursorImpl::loadFromSystem(Cursor::Type type)
{
    release();
