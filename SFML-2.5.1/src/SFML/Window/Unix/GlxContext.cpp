...
int
HandleXError(::Display*, XErrorEvent*)
{
...
////////////////////////////////////////////////////////////
void
ensureExtensionsInit(::Display* display, int screen)
{
...
////////////////////////////////////////////////////////////
GlFunctionPointer
GlxContext::getFunction(const char* name)
{
...
////////////////////////////////////////////////////////////
bool
GlxContext::makeCurrent(bool current)
{
    if (!m_context)
        return false;
...
////////////////////////////////////////////////////////////
void
GlxContext::setVerticalSyncEnabled(bool enabled)
{
    int result = 0;
...
////////////////////////////////////////////////////////////
XVisualInfo
GlxContext::selectBestVisual(::Display* display,
                             unsigned int bitsPerPixel,
                             const ContextSettings& settings)
{
    // Make sure that extensions are initialized
    ...
    if (visuals)
    {
        ...
        for (int i = 0; i < count; ++i)
        {
            ...
            if ((sfglx_ext_EXT_framebuffer_sRGB == sfglx_LOAD_SUCCEEDED) ||
                (sfglx_ext_ARB_framebuffer_sRGB == sfglx_LOAD_SUCCEEDED))
            {
...
////////////////////////////////////////////////////////////
void
GlxContext::updateSettingsFromVisualInfo(XVisualInfo* visualInfo)
{
...
////////////////////////////////////////////////////////////
void
GlxContext::updateSettingsFromWindow()
{
    ...
    XVisualInfo* visualInfo =
        XGetVisualInfo(m_display, VisualIDMask | VisualScreenMask, &tpl, &nbVisuals);
...
////////////////////////////////////////////////////////////
void
GlxContext::createSurface(GlxContext* shared,
                          unsigned int width,
                          unsigned int height,
                          unsigned int bitsPerPixel)
{
    ...
    if (shared && (sfglx_ext_SGIX_pbuffer == sfglx_LOAD_SUCCEEDED))
    {
        ...
        if (hasCreatePbuffer)
        {
            ...
            int nbConfigs = 0;
            GLXFBConfig* configs =
                glXChooseFBConfig(m_display, DefaultScreen(m_display), NULL, &nbConfigs);
            ...
            if (config)
            {
                int attributes[] = { GLX_PBUFFER_WIDTH,
    ...
    attributes.colormap =
        XCreateColormap(m_display, RootWindow(m_display, screen), visualInfo.visual, AllocNone);
...
////////////////////////////////////////////////////////////
void
GlxContext::createSurface(::Window window)
{
...
GlxContext::createContext(GlxContext* shared)
{
    ...
    if (m_pbuffer)
    {
        ...
        GLXFBConfig* fbconfig =
            glXChooseFBConfig(m_display, DefaultScreen(m_display), attributes, &count);
    ...
    bool hasCreateContextArb =
        (sfglx_ext_ARB_create_context == sfglx_LOAD_SUCCEEDED) && ((major > 1) || (minor >= 3));
    ...
    if (hasCreateContextArb)
    {
        ...
        if (!config)
            err() << "Failed to get GLXFBConfig which corresponds to the window's visual"
                  << std::endl;
        ...
        while (config && !m_context && m_settings.majorVersion)
        {
            ...
            // Create the context
            m_context =
                glXCreateContextAttribsARB(m_display, *config, toShare, true, &attributes[0]);

            if (!m_context)
            {
                // If we couldn't create the context, first try disabling flags,
                // then lower the version number and try again -- stop at 0.0
                // Invalid version numbers will be generated by this algorithm (like 3.9), but we
                // really don't care
