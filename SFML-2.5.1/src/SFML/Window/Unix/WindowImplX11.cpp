...
struct KeyRepeatFinder
{
    ...
    {
        return ((event.type == KeyPress) && (event.xkey.keycode == keycode) &&
...
Bool
checkEvent(::Display*, XEvent* event, XPointer userData)
{
...
::Window
getParentWindow(::Display* disp, ::Window win)
{
    ::Window root, parent;
...
bool
getEWMHFrameExtents(::Display* disp, ::Window win, long& xFrameExtent, long& yFrameExtent)
{
    if (!ewmhSupported())
        return false;
    ...
    if ((result == Success) && (actualType == XA_CARDINAL) && (actualFormat == 32) &&
        (numItems == 4) && (numBytesLeft == 0) && (data != NULL))
    {
  ...
  , m_lastInputTime(0)
{
    ...
    XSetWindowAttributes attributes;
    attributes.colormap =
        XCreateColormap(m_display, DefaultRootWindow(m_display), visualInfo.visual, AllocNone);
    attributes.event_mask = eventMask;
...
////////////////////////////////////////////////////////////
WindowHandle
WindowImplX11::getSystemHandle() const
{
    return m_window;
}
...
////////////////////////////////////////////////////////////
Vector2i
WindowImplX11::getPosition() const
{
...
{
    // If resizing is disable for the window we have to update the size hints (required by some
    // window managers).
    if (m_useSizeHints)
    {
        XSizeHints* sizeHints = XAllocSizeHints();
...
////////////////////////////////////////////////////////////
void
WindowImplX11::setIcon(unsigned int width, unsigned int height, const Uint8* pixels)
{
    ...
    XImage* iconImage = XCreateImage(
        m_display, defVisual, defDepth, ZPixmap, 0, (char*)iconPixels, width, height, 32, 0);
    if (!iconImage)
    {
    ...
    m_iconPixmap =
        XCreatePixmap(m_display, RootWindow(m_display, m_screen), width, height, defDepth);
    XGCValues values;
    ...
    m_iconMaskPixmap = XCreatePixmapFromBitmapData(
        m_display, m_window, (char*)&maskPixels[0], width, height, 1, 0, 1);
...
WindowImplX11::setMouseCursorGrabbed(bool grabbed)
{
    ...
    if (grabbed)
    {
        ...
        {
            int result = XGrabPointer(m_display,
                                      m_window,
                                      True,
                                      None,
                                      GrabModeAsync,
...
WindowImplX11::requestFocus()
{
    ...
    {
        Lock lock(allWindowsMutex);
        for (std::vector<WindowImplX11*>::iterator itr = allWindows.begin();
             itr != allWindows.end();
             ++itr)
        {
            if ((*itr)->hasFocus())
...
WindowImplX11::grabFocus()
{
    ...
    if (netActiveWindow)
    {
        ...
        if (!result)
            err() << "Setting fullscreen failed, could not send \"_NET_ACTIVE_WINDOW\" event"
                  << std::endl;
    }
    else
    {
...
WindowImplX11::setVideoMode(const VideoMode& mode)
{
    ...
    if (!res)
    {
        err() << "Failed to get the current screen resources for fullscreen mode, switching to "
                 "window mode"
              << std::endl;
        return;
    }
    ...
    if (!outputInfo || outputInfo->connection == RR_Disconnected)
    {
        ...
        err() << "Failed to get output info for fullscreen mode, switching to window mode"
              << std::endl;
        return;
    }
    ...
    if (!crtcInfo)
    {
        XRRFreeScreenResources(res);
        XRRFreeOutputInfo(outputInfo);
        err() << "Failed to get crtc info for fullscreen mode, switching to window mode"
              << std::endl;
        return;
    }
    ...
    if (!modeFound)
    {
        XRRFreeScreenResources(res);
        XRRFreeOutputInfo(outputInfo);
        err() << "Failed to find a matching RRMode for fullscreen mode, switching to window mode"
              << std::endl;
        return;
    }
    ...
    if (fullscreenWindow == this)
    {
        ...
        if (checkXRandR(xRandRMajor, xRandRMinor))
        {
            XRRScreenResources* res =
                XRRGetScreenResources(m_display, DefaultRootWindow(m_display));
            if (!res)
            {
                err() << "Failed to get the current screen resources to reset the video mode"
                      << std::endl;
                return;
            }
...
WindowImplX11::switchToFullscreen()
{
    ...
    if (ewmhSupported())
    {
        ...
        if (!result)
            err() << "Setting fullscreen failed, could not send \"_NET_WM_STATE\" event"
                  << std::endl;
    }
}
...
WindowImplX11::initialize()
{
    ...
    if (!m_inputContext)
        err() << "Failed to create input context for window -- TextEntered event won't be able to "
                 "return unicode"
              << std::endl;
...
////////////////////////////////////////////////////////////
void
WindowImplX11::updateLastInputTime(::Time time)
{
...
WindowImplX11::createHiddenCursor()
{
    ...
    m_hiddenCursor =
        XCreatePixmapCursor(m_display, cursorPixmap, cursorPixmap, &color, &color, 0, 0);
...
////////////////////////////////////////////////////////////
bool
WindowImplX11::processEvent(XEvent& windowEvent)
{
    ...
    if (windowEvent.type == KeyRelease)
    {
        ...
        std::deque<XEvent>::iterator iter =
            std::find_if(m_events.begin(),
                         m_events.end(),
    ...
    switch (windowEvent.type)
    {
        ...
        case FocusIn:
        {
            ...
            // If the window has been previously marked urgent (notification) as a result of a focus
            // request, undo that
            XWMHints* hints = XGetWMHints(m_display, m_window);
            if (hints != NULL)
            {
        ...
        {
            // ConfigureNotify can be triggered for other reasons, check if the size has actually
            // changed
            if ((windowEvent.xconfigure.width != m_previousSize.x) ||
                (windowEvent.xconfigure.height != m_previousSize.y))
            {
                Event event;
        ...
        case KeyPress:
        {
            ...
            // TODO: if modifiers are wrong, use XGetModifierMapping to retrieve the actual
            // modifiers mapping
            Event event;
            ...
            if (!XFilterEvent(&windowEvent, None))
            {
...
#endif
                {
                    ...
                    char keyBuffer[16];
                    if (XLookupString(
                            &windowEvent.xkey, keyBuffer, sizeof(keyBuffer), NULL, &status))
                    {
                        Event textEvent;
                        textEvent.type = Event::TextEntered;
...
WindowImplX11::checkXRandR(int& xRandRMajor, int& xRandRMinor)
{
    ...
    if (!XRRQueryVersion(m_display, &xRandRMajor, &xRandRMinor) || xRandRMajor < 1 ||
...
////////////////////////////////////////////////////////////
RROutput
WindowImplX11::getOutputPrimary(::Window& rootWindow,
                                XRRScreenResources* res,
                                int xRandRMajor,
                                int xRandRMinor)
{
...
WindowImplX11::getPrimaryMonitorPosition()
{
    ...
    if (!res)
    {
        err() << "Failed to get the current screen resources for.primary monitor position"
              << std::endl;
