...
VideoModeImpl::getFullscreenModes()
{
    ...
    if (display)
    {
        ...
        if (XQueryExtension(display, "RANDR", &version, &version, &version))
        {
            ...
            else
            {
                ...
                err() << "Failed to retrieve the screen configuration while trying to get the "
        ...
        else
        {
            ...
            err() << "Failed to use the XRandR extension while trying to get the supported video "
                     "modes"
                  << std::endl;
        }
    ...
    else
    {
        ...
        err() << "Failed to connect to the X server while trying to get the supported video modes"
              << std::endl;
    }
...
VideoModeImpl::getDesktopMode()
{
    ...
    if (display)
    {
        ...
        if (XQueryExtension(display, "RANDR", &version, &version, &version))
        {
            ...
            if (config)
            {
                ...
                if (sizes && (nbSizes > 0))
                {
                    desktopMode = VideoMode(sizes[currentMode].width,
    ...
    {
        // We couldn't connect to the X server
        err() << "Failed to connect to the X server while trying to get the desktop video modes"
              << std::endl;
    }
