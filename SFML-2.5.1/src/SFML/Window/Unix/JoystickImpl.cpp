...
bool
isJoystick(udev_device* udevDevice)
{
...
void
updatePluggedList(udev_device* udevDevice = NULL)
{
    if (udevDevice)
    {
            ...
            if (isJoystick(udevDevice))
            {
                ...
                if (record == joystickList.end())
                {
                    ...
                    else if (std::strstr(action, "remove"))
                    {
                        ...
                        sf::err() << "Trying to disconnect joystick that wasn't connected"
                                  << std::endl;
                    }
                }
            }
    ...
    for (JoystickList::iterator record = joystickList.begin(); record != joystickList.end();
         ++record)
        record->plugged = false;
...
const char*
getUdevAttribute(udev_device* udevDevice, const std::string& attributeName)
{
...
const char*
getUsbAttribute(udev_device* udevDevice, const std::string& attributeName)
{
...
unsigned int
getUsbAttributeUint(udev_device* udevDevice, const std::string& attributeName)
{
    if (!udevDevice)
        return 0;
...
unsigned int
getUdevAttributeUint(udev_device* udevDevice, const std::string& attributeName)
{
    if (!udevDevice)
        return 0;
...
unsigned int
getJoystickVendorId(unsigned int index)
{
    if (!udevContext)
    {
        sf::err() << "Failed to get vendor ID of joystick " << joystickList[index].deviceNode
                  << std::endl;
        return 0;
    }

    udev_device* udevDevice =
        udev_device_new_from_syspath(udevContext, joystickList[index].systemPath.c_str());
...
unsigned int
getJoystickProductId(unsigned int index)
{
    if (!udevContext)
    {
        sf::err() << "Failed to get product ID of joystick " << joystickList[index].deviceNode
                  << std::endl;
        return 0;
    }
...
std::string
getJoystickName(unsigned int index)
{
...
JoystickImpl::initialize()
{
    ...
    if (!udevMonitor)
    {
        err() << "Failed to create udev monitor, joystick connections and disconnections won't be "
                 "notified"
              << std::endl;
    }
    else
    {
        ...
        if (error < 0)
        {
            err() << "Failed to add udev monitor filter, joystick connections and disconnections "
                     "won't be notified: "
                  << error << std::endl;
        ...
        else
        {
            ...
            if (error < 0)
            {
                err() << "Failed to enable udev monitor, joystick connections and disconnections "
                         "won't be notified: "
                      << error << std::endl;
...
////////////////////////////////////////////////////////////
JoystickState
JoystickImpl::JoystickImpl::update()
{
    if (m_file < 0)
    {
        m_state = JoystickState();
