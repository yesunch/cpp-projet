...
class WindowImplX11 : public WindowImpl
{
  ...
  private:
    ...
    ::Cursor
        m_hiddenCursor; ///< As X11 doesn't provide cursor hiding, we must create a transparent one
    ::Cursor m_lastCursor;   ///< Last cursor used -- this data is not owned by the window and is
                             ///< required to be always valid
    bool m_keyRepeat;        ///< Is the KeyRepeat feature enabled?
    Vector2i m_previousSize; ///< Previous size of the window, to find if a ConfigureNotify event is
                             ///< a resize event (could be a move event only)
