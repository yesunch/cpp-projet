...
////////////////////////////////////////////////////////////
void
ensureExtensionsInit(HDC deviceContext)
{
...
////////////////////////////////////////////////////////////
String
getErrorString(DWORD errorCode)
{
...
////////////////////////////////////////////////////////////
GlFunctionPointer
WglContext::getFunction(const char* name)
{
    GlFunctionPointer address =
        reinterpret_cast<GlFunctionPointer>(wglGetProcAddress(reinterpret_cast<LPCSTR>(name)));
    ...
    if (module)
        return reinterpret_cast<GlFunctionPointer>(
            GetProcAddress(module, reinterpret_cast<LPCSTR>(name)));
...
////////////////////////////////////////////////////////////
void
WglContext::setVerticalSyncEnabled(bool enabled)
{
    ...
    if (sfwgl_ext_EXT_swap_control == sfwgl_LOAD_SUCCEEDED)
    {
        if (wglSwapIntervalEXT(enabled ? 1 : 0) == FALSE)
            err() << "Setting vertical sync failed: "
...
////////////////////////////////////////////////////////////
int
WglContext::selectBestPixelFormat(HDC deviceContext,
                                  unsigned int bitsPerPixel,
                                  const ContextSettings& settings,
                                  bool pbuffer)
{
    ...
    if (sfwgl_ext_ARB_pixel_format == sfwgl_LOAD_SUCCEEDED)
    {
        ...
        bool isValid = wglChoosePixelFormatARB(
                           deviceContext, intAttributes, NULL, 512, formats, &nbFormats) != FALSE;

        if (!isValid)
            err() << "Failed to enumerate pixel formats: "
                  << getErrorString(GetLastError()).toAnsiString() << std::endl;
        ...
        if (isValid && (nbFormats > 0))
        {
            ...
            for (UINT i = 0; i < nbFormats; ++i)
            {
                ...
                int values[7];
                const int attributes[] = { WGL_RED_BITS_ARB,    WGL_GREEN_BITS_ARB,
                                           WGL_BLUE_BITS_ARB,   WGL_ALPHA_BITS_ARB,
                                           WGL_DEPTH_BITS_ARB,  WGL_STENCIL_BITS_ARB,
                                           WGL_ACCELERATION_ARB };

                if (wglGetPixelFormatAttribivARB(
                        deviceContext, formats[i], PFD_MAIN_PLANE, 7, attributes, values) == FALSE)
                {
                ...
                if ((sfwgl_ext_ARB_framebuffer_sRGB == sfwgl_LOAD_SUCCEEDED) ||
                    (sfwgl_ext_EXT_framebuffer_sRGB == sfwgl_LOAD_SUCCEEDED))
                {
                    ...
                    if (wglGetPixelFormatAttribivARB(deviceContext,
                                                     ...
                                                     PFD_MAIN_PLANE,
                                                     1,
                                                     &sRgbCapableAttribute,
                                                     &sRgbCapableValue) == FALSE)
                    {
...
////////////////////////////////////////////////////////////
void
WglContext::setDevicePixelFormat(unsigned int bitsPerPixel)
{
    ...
    {
        err() << "Failed to set pixel format for device context: "
              << getErrorString(GetLastError()).toAnsiString() << std::endl
              << "Cannot create OpenGL context" << std::endl;
        return;
    }
...
////////////////////////////////////////////////////////////
void
WglContext::updateSettingsFromPixelFormat()
{
    int format = GetPixelFormat(m_deviceContext);

    if (format == 0)
    {
        err() << "Failed to get selected pixel format: "
              << getErrorString(GetLastError()).toAnsiString() << std::endl;
        return;
    }
    ...
    if (sfwgl_ext_ARB_pixel_format == sfwgl_LOAD_SUCCEEDED)
    {
        ...
        if (wglGetPixelFormatAttribivARB(
                m_deviceContext, format, PFD_MAIN_PLANE, 2, attributes, values) == TRUE)
        {
        ...
        if (sfwgl_ext_ARB_multisample == sfwgl_LOAD_SUCCEEDED)
        {
            ...
            if (wglGetPixelFormatAttribivARB(
                    m_deviceContext, format, PFD_MAIN_PLANE, 2, sampleAttributes, sampleValues) ==
                TRUE)
            {
...
////////////////////////////////////////////////////////////
void
WglContext::createSurface(WglContext* shared,
                          unsigned int width,
                          unsigned int height,
                          unsigned int bitsPerPixel)
{
    ...
    {
        int bestFormat =
            selectBestPixelFormat(shared->m_deviceContext, bitsPerPixel, m_settings, true);
        ...
        if (bestFormat > 0)
        {
            ...
            m_pbuffer =
                wglCreatePbufferARB(shared->m_deviceContext, bestFormat, width, height, attributes);
            ...
            {
                err() << "Failed to create pixel buffer: "
                      << getErrorString(GetLastError()).toAnsiString() << std::endl;
            }
        }
    }
...
////////////////////////////////////////////////////////////
void
WglContext::createSurface(HWND window, unsigned int bitsPerPixel)
{
    m_window = window;
...
////////////////////////////////////////////////////////////
void
WglContext::createContext(WglContext* shared)
{
    ...
    while (!m_context && m_settings.majorVersion)
    {
        if (sfwgl_ext_ARB_create_context == sfwgl_LOAD_SUCCEEDED)
        {
            ...
            if ((m_settings.majorVersion > 1) ||
                ((m_settings.majorVersion == 1) && (m_settings.minorVersion > 1)))
            {
            ...
            if (sfwgl_ext_ARB_create_context_profile == sfwgl_LOAD_SUCCEEDED)
            {
                ...
                int debug = (m_settings.attributeFlags & ContextSettings::Debug)
        ...
        // Invalid version numbers will be generated by this algorithm (like 3.9), but we really
        // don't care
        if (!m_context)
        {
    ...
    if (!m_context)
    {
        ...
        if (sharedContext)
        {
            ...
            if (wglShareLists(sharedContext, m_context) == FALSE)
                err() << "Failed to share the OpenGL context: "
                      << getErrorString(GetLastError()).toAnsiString() << std::endl;
        }
    }
}
