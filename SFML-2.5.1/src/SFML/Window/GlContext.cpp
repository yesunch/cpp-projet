...
bool
parseVersionString(const char* version,
                   const char* prefix,
                   unsigned int& major,
                   unsigned int& minor)
{
    ...
    if ((std::strlen(version) >= (prefixLength + 3)) &&
        (std::strncmp(version, prefix, prefixLength) == 0) && std::isdigit(version[prefixLength]) &&
        (version[prefixLength + 1] == '.') && std::isdigit(version[prefixLength + 2]))
    {
...
////////////////////////////////////////////////////////////
void
GlContext::registerContextDestroyCallback(ContextDestroyCallback callback, void* arg)
{
...
GlContext::create()
{
    // Make sure that there's an active context (context creation may need extensions, and thus a
    // valid context)
...
////////////////////////////////////////////////////////////
GlContext*
GlContext::create(const ContextSettings& settings,
...
////////////////////////////////////////////////////////////
GlContext*
GlContext::create(const ContextSettings& settings, unsigned int width, unsigned int height)
{
    // Make sure that there's an active context (context creation may need extensions, and thus a
    // valid context)
    assert(sharedContext != NULL);
...
////////////////////////////////////////////////////////////
bool
GlContext::isExtensionAvailable(const char* name)
{
...
////////////////////////////////////////////////////////////
GlFunctionPointer
GlContext::getFunction(const char* name)
{
#if !defined(SFML_OPENGL_ES)
...
////////////////////////////////////////////////////////////
bool
GlContext::setActive(bool active)
{
    if (active)
    {
        if (this != currentContext)
...
////////////////////////////////////////////////////////////
int
GlContext::evaluateFormat(unsigned int bitsPerPixel,
...
////////////////////////////////////////////////////////////
void
GlContext::cleanupUnsharedResources()
{
    ...
    for (ContextDestroyCallbacks::iterator iter = contextDestroyCallbacks.begin();
         iter != contextDestroyCallbacks.end();
         ++iter)
...
////////////////////////////////////////////////////////////
void
GlContext::initialize(const ContextSettings& requestedSettings)
{
    // Activate the context
    setActive(true);
    ...
    else
    {
        ...
        if (version)
        {
            // OpenGL ES Common Lite profile: The beginning of the returned string is "OpenGL ES-CL
            // major.minor" OpenGL ES Common profile:      The beginning of the returned string is
            // "OpenGL ES-CM major.minor" OpenGL ES Full profile:        The beginning of the
            // returned string is "OpenGL ES major.minor" Desktop OpenGL:                The
            // beginning of the returned string is "major.minor"

            if (!parseVersionString(
                    version, "OpenGL ES-CL ", m_settings.majorVersion, m_settings.minorVersion) &&
                !parseVersionString(
                    version, "OpenGL ES-CM ", m_settings.majorVersion, m_settings.minorVersion) &&
                !parseVersionString(
                    version, "OpenGL ES ", m_settings.majorVersion, m_settings.minorVersion) &&
    ...
    // If a 3.0 context was created with wglCreateContextAttribsARB, it is guaranteed to be
    // compatibility. If a 3.1 context was created with wglCreateContextAttribsARB, the
    // compatibility flag is set only if ARB_compatibility is present If a 3.2+ context was created
    // with wglCreateContextAttribsARB, the compatibility flag would have been set correctly already
    // depending on whether ARB_create_context_profile is supported.

    // If the user requests a 3.0 context, it will be a compatibility context regardless of the
    // requested profile. If the user requests a 3.1 context and its creation was successful, the
    // specification states that it will not be a compatibility profile context regardless of the
    // requested profile unless ARB_compatibility is present.
...
////////////////////////////////////////////////////////////
void
GlContext::checkSettings(const ContextSettings& requestedSettings)
{
    ...
    if ((m_settings.attributeFlags != requestedSettings.attributeFlags) ||
        (version < requestedVersion) || (m_settings.stencilBits < requestedSettings.stencilBits) ||
    ...
    {
        err() << "Warning: The created OpenGL context does not fully meet the settings that were "
                 "requested"
              << std::endl;
        err() << "Requested: version = " << requestedSettings.majorVersion << "."
              << requestedSettings.minorVersion << " ; depth bits = " << requestedSettings.depthBits
